...
import com.ichi2.anki.speech.WhisperEngine
...
open class Reviewer :
    ...
    private val whisper_model_manager by lazy { WhisperModelManager(this) }
    private val whisper_engine by lazy { WhisperEngine() }
    private var whisper_audio_record: AudioRecord? = null
    private var whisper_recording_job: Job? = null
    private var whisper_recording_chunks: MutableList<ShortArray> = mutableListOf()
    ...
    private fun start_speech_recognition() {
        ...
        val model_path = whisper_model_manager.active_model_path()
        if (model_path.isNullOrBlank()) {
            speech_diff_error?.text = getString(R.string.whisper_model_status_not_ready)
            return
        }
        if (!whisper_engine.init(model_path)) {
            speech_diff_error?.text = getString(R.string.whisper_engine_init_failed)
            return
        }
        ...
        if (min_buffer <= 0) {
            speech_diff_error?.text = getString(R.string.whisper_recording_failed, "min buffer error")
            return
        }
        val audio_record =
            AudioRecord(
        ...
        if (audio_record.state != AudioRecord.STATE_INITIALIZED) {
            speech_diff_error?.text = getString(R.string.whisper_recording_failed, "init failed")
            audio_record.release()
            return
        }
        whisper_audio_record = audio_record
        whisper_recording_chunks = mutableListOf()
        ...
        whisper_recording_job =
            lifecycleScope.launch(Dispatchers.IO) {
                val buffer = ShortArray(min_buffer)
                runCatching { audio_record.startRecording() }
                    .onFailure { Timber.w(it, "audio record start failed") }
                while (speech_recognition_running && audio_record.recordingState == AudioRecord.RECORDSTATE_RECORDING) {
                    val read = audio_record.read(buffer, 0, buffer.size)
                    if (read > 0) {
                        whisper_recording_chunks.add(buffer.copyOf(read))
                    }
                }
            }
    }
    ...
    private fun stop_speech_recognition() {
        ...
        update_speech_diff_button()
        val audio_record = whisper_audio_record
        whisper_audio_record = null
        val job = whisper_recording_job
        whisper_recording_job = null
        try {
            audio_record?.stop()
        } catch (e: Exception) {
            Timber.w(e, "audio record stop failed")
        }
        try {
            audio_record?.release()
        } catch (e: Exception) {
            Timber.w(e, "audio record release failed")
        }
        speech_diff_status?.text = getString(R.string.whisper_transcribing)
        lifecycleScope.launch(Dispatchers.IO) {
            runCatching { job?.join() }
            val pcm = flatten_whisper_chunks(whisper_recording_chunks)
            val text = whisper_engine.transcribe_pcm16(pcm)
    ...
    private fun release_speech_recognizer() {
        speech_recognition_running = false
        try {
            whisper_audio_record?.stop()
        } catch (_: Exception) {
        }
        try {
            whisper_audio_record?.release()
        } catch (_: Exception) {
        }
        whisper_audio_record = null
        whisper_recording_job?.cancel()
        whisper_recording_job = null
        whisper_engine.close()
    }

    private fun flatten_whisper_chunks(chunks: List<ShortArray>): ShortArray {
    ...
    private fun render_speech_diff() {
        ...
        val diffs = dmp.diffMain(expected, actual)
        val html = dmp.diffPrettyHtml(diffs)
        result_view.text = HtmlCompat.fromHtml(html, HtmlCompat.FROM_HTML_MODE_LEGACY)
    }

    // diff 逻辑已改为 diff_match_patch，见 render_speech_diff()
